    ...
    public partial class Form2 : Form
    {
        ...
        private void button209_Click(object sender, EventArgs e)
        {
            if (Program.admin == false)
            {
                ...
                Tuple<int, string> temp1 = new Tuple<int, string>(Program.counter, Program.logged_in);
                Program.counter++;
                Tuple<int, int> temp = new Tuple<int, int>(normal_counter + vip_counter, sum);
                if (sum != 0)
                {
                    Program.purchases.Add(temp1, temp);
                }
                Program.match1_all_normal_counter += normal_counter;
                Program.match1_all_vip_counter += vip_counter;
                Program.match1_sum += sum;
        ...
        private void button212_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (KeyValuePair<Tuple<int, string>, Tuple<int, int>> kvp in Program.purchases)
                {
                    textBox1.Text += kvp.Key.Item1 + ". Buyer: " + kvp.Key.Item2 + " Tickets: " + kvp.Value.Item1 + " Paid amount:" + kvp.Value.Item2+"\r\n";
                }
                textBox1.Text += "Value of normal tickets bought is:" + Program.match1_all_normal_counter * 15000 + "\tValue of vip tickets bought is:" + Program.match1_all_vip_counter * 50000+ "\r\n";
                textBox1.Text += "Value of all tickets bought is: " + Program.match1_sum;
            }
            else
            {
